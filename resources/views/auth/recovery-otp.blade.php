...
@section('title', 'TravelCare - Password Recovery')

@section('content')
    <div class="form-container">
        <div class="auth-content-box">
            <a href="{{ route('recover', ['recovery_type' => $recovery_type]) }}" id="prevButton"><img src="/assets/icons/arrow-left.svg" alt="return" class="arr-back"></a>
            
            <!-- Password Recovery OTP Box -->
            <div class="signin-content" id="box4">
                <h2 class="title secondary">Enter Verification Code</h2>
                <p>We've sent a verification code to {{ $recovery_type === 'email' ? 'your email' : 'your phone number' }} <br> 
                    <span class="contact-info">{{ $contact_info }}</span>
                </p>
                ...
                <form action="{{ route('verify-recovery-otp') }}" method="POST" id="otpForm">
                    @csrf
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        resendBtn.addEventListener('click', function() {
            if (!this.disabled) {
                ...
                fetch('{{ route("resend-recovery-otp") }}', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Do not display verification code in UI
        ...
        // Removed testing auto-fill; user must enter the code manually
    });
</script>
@endsection